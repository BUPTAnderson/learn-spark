...
object RddDataframeDataset {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Spark能够以二进制的形式序列化数据(不包括结构)到off-heap中, 当要操作数据时, 就直接操作off-heap内存. 由于Spark理解schema, 所以知道该如何操作.
    ...
    //-------------------------------------------- dataset ------------------------------------------------------
    // RDD API 存储的是原始的 JVM 对象，提供丰富的函数式运算符，使用起来很灵活，但是由于缺乏类型信息很难对它的执行过程优化。
    // DataFrame API 存储的是 Row 对象，提供了基于表达式的运算以及逻辑查询计划，很方便做优化，并且执行起来速度很快，但是却不如 RDD API 灵活。
    // DataSet API 则充分结合了二者的优势，既允许用户很方便的操作领域对象又拥有 SQL 执行引擎的高性能表现。
    // 本质上来说 DataSet API 相当于 RDD + Encoder， Encoder 可以将原始的 JVM对象高效的转化为二进制格式，使得可以后续对其进行更多的处理。
  }
}
